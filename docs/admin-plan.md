...
Account facts still to confirm during phase 1: the actual media storage provider, chosen admin hostname, allowed owner identity and MFA provider, available protection rules for `V1`, the beta deploy integration, and Images/R2 availability. The publishing base is not undecided: it is fixed to `V1`. Repository inspection alone does not verify the external deployment and account settings.
