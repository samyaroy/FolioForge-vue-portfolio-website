...
Status: React + Vite + TypeScript + shadcn/ui frontend and a separate fail-closed Cloudflare Worker security foundation are implemented locally in `admin/`. Wrangler targets `admin.samyabrata.codeium.xyz`; Access JWT verification, owner authorization, asset protection, session-bound CSRF, and beta-only policy have local security tests. Owner identity and the Access audience are Worker secrets, absent from this public repository, and denials are logged for the live gate. The live Access policy and account setup still require verification. Persisted editing, R2, GitHub, and publishing remain planned. No infrastructure has been deployed. See `admin/worker/README.md` for the setup, the deployment order, and the live security gate.
