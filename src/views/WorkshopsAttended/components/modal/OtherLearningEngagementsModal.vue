  ...
  <Teleport to="body">
    <Transition name="modal-fade">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="other-learning-engagements-title"
        ...
        <div class="flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-lg bg-slate-50 shadow-2xl">
          <header class="flex items-start justify-between gap-4 border-b border-slate-200 bg-white px-5 py-4">
            <div>
              ...
              <h2 id="other-learning-engagements-title" class="mt-1 text-2xl font-black text-[#0e141b]">
                Other Learning Engagements
              </h2>
            </div>
          ...
          <div class="min-h-0 flex-1 overflow-y-auto px-5 py-5">
            <div v-if="othersByYear.length > 0" class="mx-auto max-w-4xl">
              <section v-for="yearGroup in othersByYear" :key="yearGroup.year" class="mb-10 last:mb-0">
            ...
            <div v-else class="py-12 text-center">
              <p class="text-lg text-gray-500">No other learning engagements found</p>
            </div>
          </div>
        </div>
      </div>
...
const othersByYear = computed(() => groupByYear(props.engagements))
</script>
