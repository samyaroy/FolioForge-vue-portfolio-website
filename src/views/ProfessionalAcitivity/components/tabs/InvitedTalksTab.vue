<template>
  <div class="max-w-4xl mx-auto">
    <div v-if="talksByYear.length > 0">
      <div v-for="yearGroup in talksByYear" :key="yearGroup.year" class="mb-12">
        <!-- Year Divider -->
        ...
        <!-- Cards for this year -->
        <div class="space-y-6">
          <InvitedTalkCard v-for="(talk, index) in yearGroup.items" 
                        :key="index" :talk="talk" />
        </div>
      </div>
    </div>
    ...
    <div v-else class="text-center py-12">
      <p class="text-gray-500 text-lg">No invited talks yet</p>
    </div>
  </div>
</template>
...
import InvitedTalkCard from '../cards/InvitedTalkCard.vue'
...
const talksByYear = computed(() => groupByYear(props.talks))
</script>
