<template>
    <div class="bg-white rounded-lg shadow-sm p-8">
        <h2 class="text-2xl font-bold text-[#0e141b] mb-6">Theoretical Projects / Research Projects</h2>
        <div v-if="projects && projects.length > 0" class="space-y-6">
            <div v-for="project in projects" :key="project.id" class="border-l-4 border-[#1980e6] pl-6 py-4">
                ...
                <p class="text-gray-600 mb-2"><v-icon>mdi-account-badge</v-icon> Supervisior: {{ project.guide }}</p>
                <p class="text-sm text-gray-600 mb-3"><v-icon>mdi-calendar-blank</v-icon> {{ project.time_period }}</p>
