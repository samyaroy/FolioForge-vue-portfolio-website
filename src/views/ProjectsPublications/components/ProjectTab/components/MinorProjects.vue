<template>
    <div class="bg-white rounded-lg shadow-sm p-4 sm:p-8">
        <button type="button"
            ...
            @click="isOpen = !isOpen">
            <h2 class="text-2xl font-bold text-[#0e141b]">Minor Projects</h2>
        ...
        <div v-if="projects && projects.length" class="space-y-6 text-sm">
            <div v-for="(project, index) in projects" :key="project.title || index" :id="`minor-${index}`"
                ...
                :aria-label="isOpen ? undefined : 'Expand minor projects section'"
                @click="expandSection"
                ...
                <div class="flex flex-col md:flex-row items-stretch">
                    <div class="w-full md:pr-6" :class="getLogos(project.logo).length ? 'md:w-[96%]' : 'md:w-full'">
                        <div v-if="project.affiliation" class="flex items-start gap-1 mb-2">
                    ...
                    <div v-if="getLogos(project.logo).length"
                        class="w-full md:w-[4%] flex flex-col items-start justify-start gap-1 md:items-center mr-3 mt-4 md:mt-0 md:overflow-visible">
                        <img v-for="logo in getLogos(project.logo)" :key="logo" :src="getLogoPath(logo)" :alt="logo"
                            :title="logo" class="max-h-8 w-auto object-contain opacity-100 md:max-h-14 md:max-w-none">
                    </div>
                </div>
...
const isOpen = ref(isFeatureEnabled('showProjectsPublications.expandProjectSectionsByDefault.minorProjects'))
