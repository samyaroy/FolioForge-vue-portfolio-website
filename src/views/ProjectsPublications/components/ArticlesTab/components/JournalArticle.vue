    ...
    <div v-if="articles.length" class="space-y-6 text-sm">
      <div v-for="(article, index) in articles" :key="article.title || index"
        ...
        <div class="flex items-stretch gap-4">
          <div class="w-[14%] shrink-0 self-stretch flex items-center gap-2 border-r-2 border-gray-500 pr-2">
            <img v-for="logo in article.logo" :key="logo" :src="getLogoPath(logo)" :alt="logo" :title="logo"
              class="w-full max-h-8 md:max-h-16 object-contain opacity-90" />
          </div>
          <div class="w-[86%] min-w-0">
