...
export function MovieModal({ movie, onClose }: MovieModalProps) {
  ...
  const titleId = `movie-review-title-${movie.id}`
  const rating = movie.rating
  const posterImage = posterFailed ? undefined : movie.image
  const noteParagraphs = (movie.note || movie.description || '')
    .split(/\n\s*\n/)
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 overflow-y-auto md:grid-cols-[14rem_minmax(0,1fr)]">
          ...
          <div className="flex min-w-0 flex-col px-6 py-6">
            <div className={`${CARD_META_CLASS} mt-0`}>
              ...
              {movie.date && (
                <time dateTime={movie.date}>
                  {MOVIES_SECTION.watchedLabel} {formatDate(movie.date)}
                </time>
              )}
            </div>
